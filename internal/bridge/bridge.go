...
func qoderChatStreamURL(region qoder.Region) string {
	return qoder.GetEndpoints(region).ChatStreamURL
}

func qoderModelListURL(region qoder.Region) string {
	return qoder.GetEndpoints(region).ModelListURL
}
...
func FetchUserInfoWithToken(ctx context.Context, token string, region qoder.Region) (map[string]interface{}, error) {
	ep := qoder.GetEndpoints(region)
...
func NewBridge(ctx context.Context, pat string, region qoder.Region, templateBase map[string]interface{}) (*Bridge, error) {
	...
	if strings.HasPrefix(deviceToken, "dt-") {
		...
		jt, err := cosy.ExchangeJobToken(ctx, pat, cosy.DeriveMachineID(seed), cosy.DeriveMachineToken(seed), cosy.DeriveMachineType(seed), qoder.GetEndpoints(region).JobTokenURL)
		if err != nil {
